...
(() => {
  ...
  function positionBtn() {
    ...
    switch (settings.position) {
      ...
      case 'top-center':    top = rect.top    + off; left = rect.left  + (rect.width  - s) / 2; break;
      case 'top-right':     top = rect.top    + off; left = rect.right - s - off; break;
      case 'left-center':   top = rect.top    + (rect.height - s) / 2; left = rect.left  + off; break;
      case 'right-center':  top = rect.top    + (rect.height - s) / 2; left = rect.right - s - off; break;
      case 'bottom-left':   top = rect.bottom - s - off; left = rect.left  + off; break;
      case 'bottom-center': top = rect.bottom - s - off; left = rect.left  + (rect.width  - s) / 2; break;
      case 'bottom-right':  top = rect.bottom - s - off; left = rect.right - s - off; break;
      default:              top = rect.top    + off; left = rect.right - s - off;
    }
